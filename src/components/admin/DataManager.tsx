          ...
          <div key={table.id} className="bg-white p-8 rounded-sm border border-gray-100 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-gray-50 rounded-sm space-y-2">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">1. Descarregar Modelo</p>
                <p className="text-xs text-gray-500">Obtenha o ficheiro CSV com as colunas corretas para preencher.</p>
                <button
                  onClick={() => downloadTemplate(table.id)}
                  className="mt-2 flex items-center gap-2 text-brand-charcoal hover:text-brand-red transition-colors text-xs font-bold"
                >
                  <Download size={14} /> Descarregar template_{table.id}.csv
                </button>
              </div>
